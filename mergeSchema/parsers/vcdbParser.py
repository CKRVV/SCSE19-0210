...
import parsers.vcdbReplacement as vr
...
softwareInstalltion = "Software installation"	
...
def addIncidentDate(JSONDict):														#timeline.incident.day,timeline.incident.month,timeline.incident.year are integers						
	...
	if timeline in JSONDict:
		...
		if yearint != nullValue and monthint != nullValue and dayint != nullValue:
			estimate = No
...
def addResolutionDate(JSONDict,indicidentDateDict):								#Resolution date is calculated by adding relevant time unit and value to the incident date. uses relativedelta
	if indicidentDateDict[isEstimate] == No:
		value,unit = getResolution(JSONDict)
...
def checkDayMonth(dayint,monthint):
	estimate = No
	if monthint == 2:				#Account for february
		if dayint > 28:
			dayint = 28
...
def addIndustry(JSONDict): #victim.industry converted from SIC Codes which are integer values
	if victim in JSONDict:
		if industry in JSONDict[victim]:
			...
			# if industryTitle == nullValue:												#For error checking
				# print(JSONDict[victim][industry])
...
def replaceOrgType(industryTitle):
...
def addCountry(JSONDict):		#victim.country is a list
	if victim in JSONDict:
		if country in JSONDict[victim]:
			if Unknown in JSONDict[victim][country]:
				return nullValue
			else: 
...
def addState(JSONDict):	#victim.state is a str value
	...
	if victim in JSONDict:
		if state in JSONDict[victim]:
			if Unknown in JSONDict[victim][state] or JSONDict[victim][state] == "":
				return nullValue
			else:
...
def getAttackerName(JSONDict):	#actor.external.name actor.partner.name are lists
	actorList = []																			#Declare list for multiple actors
...
def parseAttackerRoleList(roleList):
		...
		for term in toReplaceList:
			for r in roleList:
				if r == term: 
					...
					break
	for elem in attackerRoleRemoveList:
		while elem in roleList:
			roleList.remove(elem)
	return roleList		

def addAttackerToDict(JSONDict,records_action,desc):
...
def getSpiesAndVandals(records_action,desc):
	if records_action != nullValue:
		if "Defacement" in records_action:
			return "Vandals"
	if desc != nullValue:
		if re.search(defacedRegex,desc,re.IGNORECASE): 
			return "Vandals"
		if re.search(spyRegex,desc,re.IGNORECASE):
			return "Spies"
...
def parsePurposeList(purposeList):
	...
	if action in JSONDict:
		if malware in JSONDict[action]:
			if name in JSONDict[action][malware]:
				aMalware = JSONDict[action][malware][name]		
	if aMalware != "":
		return aMalware
	else:
		return nullValue
...
def parseVectorList(vectorList):
	for vector,toReplaceList in vectorReplacementDict.items():
		for term in toReplaceList:
...
def parseVarietyList(varietyList):
...
def addTVA(JSONDict):		#attribute.integrity.variety
	...
	if integrity in JSONDict[attribute]:
			if variety in JSONDict[attribute][integrity]:
				...
				elif softwareInstalltion in JSONDict[attribute][integrity][variety]:
					fullList.append(hardwareTampering)
...
def addUnAuthResult(JSONDict,actionList): #action.hacking.result action.malware.result action.misuse.result action.physical.result action.social.result are lists
	...
	unAuthList.extend(getRecordsAction(JSONDict))
...
def getAssetsType(JSONDict):#asset.assets.variety is a list of strings where the elements have letters append like M - Document asset.management asset.ownership are list
	assetTypeList = []
	if asset in JSONDict:
		if assets in JSONDict[asset]:
			if variety in JSONDict[asset][assets][0]:
				if JSONDict[asset][assets][0][variety] != Unknown and JSONDict[asset][assets][0][variety] != other:
					parsed = JSONDict[asset][assets][0][variety].split(" - ")[1].strip()
...
def parseTargetedList(targetedList):
	for target,toReplaceList in targetReplacementDict.items():
		for term in toReplaceList:
...
def convertToUSD(amt,currency,dateDict):									#Convert the amount to USD
	...
	if currency == currencyUSD :												#If value already in USD, it does not need conversion
		...
		if currency in cc.currencies:
			if curYear != nullValue and curMonth != nullValue and curDay != nullValue:
				...
				try:
					return round(cc.convert(amt, currency, currencyUSD, date=datetime(curYear,6,15)),2)				#Take middle of the year
				except:
					return amt * mp.ratesAgainstUSD[currency]
...
def addDescription(JSONDict): #control_failure + summary are str
	...
	if control_failure in JSONDict:
		fullDesc = JSONDict[control_failure].capitalize() 				#Capitalize before assignment
	if summary in JSONDict:
		if fullDesc != "":
			fullDesc = fullDesc + "\n" + JSONDict[summary].capitalize() #Concat a breakline and Capitalize before concatenating
		else:
			fullDesc = JSONDict[summary].capitalize() 
	if fullDesc != "":
		return fullDesc
	else:
		return nullValue
...
def addSourceLink(JSONDict): #reference is a str with url separated by ; or ,
	urlList = []
	purlList = []
